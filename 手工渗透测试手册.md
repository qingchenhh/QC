...
在渗透测试过程中遇到有WAF的或者其他奇奇怪怪要求只能手工测试的（不考虑端口扫描这些，就是给个站就干这个站那种），比如吧使用sql注入，如果只是单纯的测试and 1=1，感觉太简单了，容易测漏。所以就列一个测试payload清单放在这里，就是测试的时候挨个复制粘贴看一下结果。
...
4. 手机验证码绕过测试（逻辑漏洞测试，测试其他手机号验证码、万能验证码、空验证码）。
...
常见的弱口令可以试试我自己弄的这个字典：https://github.com/qingchenhh/Simple-dictionary-generator/blob/main/dic/password.txt
...
直接dirsearch直接开干，不过速率调低就行，和手工测也没啥区别。

如果非要手工测试的话，为了测试的全面主要是看一下有没有网站备份或者是其他敏感文件或者是git泄露之类的，不需要像dirsearch一样详细的扫目录。
...
1. 如果涉及数据加密，需要逆向js进行测试，说不定有SQL注入和敏感信息泄露，具体分析的话需要靠一点基础还足够的耐心和细心。
2. 可以看看前端代码，是否带有用户名密码之类的。
3. 可以看看前端代码是否存在访问后台的一些接口以及请求的参数，搞一些未授权之类的。